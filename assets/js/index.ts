...
function percentage(group: any[], per: number) {
    var total = 0;
...
function createPercentage(chart: any, group: any, d: any) {
...
module Tips {
    function bindEventHanlder(groupName: string, seletor: string, techTip) {
    ...
    export function Tip1(title: string) {
    ...
    export function CreateTechTip(groupName: string) {
        bindEventHanlder(groupName, "g.row", this.Tip1("技术名称"));
        return this;
    }

    export function CreateBooksTip(groupName: string) {
        bindEventHanlder(groupName, "g.row", this.Tip1("书名"));
        return this;
    }

    export function PieTip() {
        return d3.tip()
            .attr('class', 'd3-tip')
            .offset([-10, 0])
    ...
    export function CreatePieTip(groupName: string) {
...
(function () {
    ...
    function filterProjects(d: IResumeDataV2) {
        return projectsHash[d.project] ? projectsHash[d.project].duration : 0;
    }

    d3.csv('data/resume-2014.csv', function (resume: any) {
        var projectNames = [], timelong = [];
        var GROUPNAME = 'work-area', ndx: any = crossfilter(resume), all = ndx.groupAll(), experChart;
        ...
        (function (_ndx) {
            ...
            //console.log(techs);
            var resumeByTechGroup = techs.group().reduceSum(function (d) { return +d.duration; });

            var techChart = dc.rowChart("#tech-area", GROUPNAME)
                .width($("#tech-area").parent().width())
                .height(810)
                .dimension(techs)
                .group(resumeByTechGroup)
                .elasticX(true)
                .title(function (d) {
                    var key = d.key,
                        value = d.value;
        ...
        (function (_ndx) {
            ...
            var resumeByCompanyGroup = companys.group().reduceSum(function (d) { return +d.duration; });

            var companysChart = dc.pieChart("#company-area", GROUPNAME)
                .width($("#company-area").parent().width())
                .height(180)
                .radius(80)
                .dimension(companys)
                .group(resumeByCompanyGroup)
                .legend(dc.legend().x(10).y(10).itemHeight(20).gap(10))
                .title(function (d) {
                    var key = d.key || (d.data && d.data.key),
                        value = d.value || (d.data && d.data.value);
        ...
        (function (_ndx) {
            ...
            var resumeByProjectGroup = projects.group().reduceSum(function (d) { return +d.duration; });

            experChart = dc.pieChart("#projectExperiences-area", GROUPNAME)
                .width($("#projectExperiences-area").parent().width())
                .height(250)
                .radius(125)
                .innerRadius(50)
                .dimension(projects)
                .group(resumeByProjectGroup)
                .legend(dc.legend().x(10).y(10).itemHeight(20).gap(10))
                .label(function (d) {
        ...
        (function (_ndx) {
            ...
            var timeChart = dc.pieChart("#year-area", GROUPNAME)
                .width($("#year-area").parent().width())
                .height(250)
                .radius(125)
                .innerRadius(50)
                .dimension(projects)
...
(function () {
    ...
    d3.csv('data/interest-2014.csv', function (resume: any) {
        var GROUPNAME = 'interest-area', ndx: any = crossfilter(resume);
        ...
        (function (_ndx) {
            ...
            var typsChart = dc.pieChart("#inter-type-area", GROUPNAME)
                .width($("#inter-type-area").parent().width())
                .height(250)
                .radius(125)
                .innerRadius(50)
                .dimension(types)
                .group(resumeByTypesGroup)
                .legend(dc.legend().x(10).y(10).itemHeight(20).gap(10))
                .title(function (d) {
                    var key = d.key || (d.data && d.data.key),
                        value = d.value || (d.data && d.data.value);
        ...
        (function (_ndx) {
            ...
            var techChart = dc.rowChart("#inter-tech-area", GROUPNAME)
                .width($("#inter-tech-area").parent().width())
                .height(600)
                .dimension(techs)
                .group(resumeByTechGroup)
                .elasticX(true)
        ...
        (function (_ndx) {
            ...
            var resumeByBooksGroup = books.group().reduceSum(function (d) { return +d.bookCount; });

            var techChart = dc.rowChart("#inter-book-area", GROUPNAME)
                .width($("#inter-book-area").parent().width())
                .height(300)
                .dimension(books)
                .group(resumeByBooksGroup)
                .elasticX(true)
                .title(function (d) {
                    var key = d.key,
                        value = d.value;
